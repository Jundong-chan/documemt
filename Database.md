...
特性（ACID）：
- **原子性**（Atomicity）：逻辑上是不可分割的操作单元，事务的所有操作要么全部提交成功，要么全部失败回滚（用回滚日志(undo log)实现，保存的是事务发生之前的版本，反向执行日志中的操作，可用于mvcc）；

  redo log 是重作日志，保存的数据修改的日志，满了会从头开始写，用于恢复数据。

  binlog 是二进制日志，mysql server层的日志，不会覆盖，可用于主从复制的记录。

- **一致性**（Consistency）：事务的执行必须使数据库保持一致性状态。在一致性状态下，所有事务对一个数据的读取结果都是相同的；

- **隔离性**（Isolation）：一个事务所做的修改在最终提交以前，对其它事务是不可见的（并发执行的事务之间不能相互影响）；

- **持久性**（Durability）：一旦事务提交成功，对数据的修改是永久性的。**提交之后也可以回滚**

### 会出现哪些并发一致性问题？
- **丢失修改**：一个事务对数据进行了修改，在事务提交之前，另一个事务对同一个数据进行了修改，覆盖了之前的修改；（**一个事物修改了另一个事务修改的但未提交的数据**）
- **脏读**（Dirty Read）：一个事务读取了被另一个事务修改、但未提交（进行了回滚）的数据，造成两个事务得到的数据不一致；（**一个事务能读取到另一个事务修改后未提交的数据**）
- **不可重复读**（Nonrepeatable Read）：在同一个事务中，某查询操作在一个时间读取某一行数据和之后一个时间读取该行数据，发现数据已经发生修改（针对**update**操作）；（**一个事务在执行过程中的两个时间点读取到的数据不一致**）
- **幻读**（Phantom Read）：当同一查询多次执行时，由于其它事务在这个数据范围内执行了插入操作，会导致每次返回不同的结果集（和不可重复读的区别：针对的是一个数据整体/范围；并且针对**insert/delete**操作）（**同一个查询执行多次返回的数据结果集不一样**）

### 数据库的四种隔离级别？

- **未提交读**（Read Uncommited）：在一个事务提交之前，它的执行结果对其它事务也是可见的。会导致脏读、不可重复读、幻读；
- **提交读**（Read Commited）：一个事务只能看见已经提交的事务所作的改变。可避免脏读问题；**读时加读锁，写时加写锁**
- **可重复读**（Repeatable Read）：可以确保同一个事务在多次读取同样的数据时得到相同的结果。（MySQL的默认隔离级别）。可避免不可重复读；**通过读的时候加共享读锁和排他写锁，保证事务过程中数据不被其他事务提交修改。**
...
- 二级封锁协议：在一级的基础上，事务在读取数据之前必须先加S锁，读完后释放。可以解决脏读问题（如果已经有事务在修改数据，就意味着已经加了X锁，此时想要读取数据的事务并不能加S锁，也就无法进行读取，避免了读取脏数据）；**读已提交**
- 三级封锁协议：在二级的基础上，事务在读取数据之前必须先加S锁，直到事务结束才能释放。可以解决不可重复读问题（避免了在事务结束前其它事务对数据加X锁进行修改，保证了事务期间数据不会被其它事务更新）**可重复读**
...
- 各种操作：
    ...
    - 查询操作时，要同时符合以下条件才能被查询出来：**删除版本号未定义或大于当前事务版本号（删除操作是在当前事务启动之后做的，比如回滚的数据）；创建版本号小于或等于当前事务版本号（创建操作是事务完成或者在事务启动之前完成）**
...
- **第三范式** 3NF：在 2NF 的基础上，非主属性**不传递依赖**于主属性（**要求一个表中不包含其他表已经存在的非主键信息**）
...
- 视图：从数据库的基本表中通过查询选取出来的数据组成的**虚拟表**（数据库中存放视图的定义）。可以对其进行增/删/改/查等操作。视图是对若干张基本表的引用，一张虚表，查询语句执行的结果，不存储具体的数据（基本表数据发生了改变，视图也会跟着改变）；**可以跟基本表一样，进行增删改查操作(ps:增删改操作有条件限制)**；如连表查询产生的视图无法进行，对视图的增删改会影响原表的数据。好处：
    ...
    - 当视图字段来自两个及以上的联合查询；字段表达式；聚集函数；orderby ; groupby等操作而来的，不允许更新
...
什么是索引：是一种对数据库中表一列或者多列进行排序的存储结构。能够极大加快检索的速度。
...
- 主键索引 PRIMARY KEY：必须唯一，不允许空值（是一种特殊的唯一索引；MySQL创建主键时默认为**聚集索引**，但主键也可以是非聚集索引）；
...
- InnoDB**支持事务**，可以进行Commit和Rollback；InnoDB每个执行语句都会当成一个事务。也可以自己将多条语句封装成一个事务。
- MyISAM 只支持表级锁，而 InnoDB 还**支持行级锁**（默认），提高了并发操作的性能；行级锁锁的是索引。
- InnoDB **支持外键**；
- MyISAM **崩溃**后发生损坏的概率比 InnoDB 高很多，而且**恢复的速度**也更慢；
- MyISAM 支持**压缩**表和空间数据索引，InnoDB需要更多的内存和存储；
- InnoDB 支持在线**热备份**
- InnoDB 主键是聚集索引，数据和主键索引的是绑定在一起的，因此主键索引中数据的排列顺序也是物理存储的排列顺序
- MyISAM用的是非聚集索引，数据和索引的分开的，索引叶子结点存放指针
- 索引结构都是B+树，即非叶子节点只存放索引值不存放数据，所有的索引都在叶子结点，使非叶子结点有更多的空间存放更多的索引，使树更矮胖。
- InnoDB不保存表的具体行数。
- InnoDB行锁锁的是索引，如果没有命中索引，则只能退化成表锁。
- 当需要使用事务，读写均衡，使用InnoDB更好一点。
...
> 分析慢查询日志：记录了在MySQL中响应时间超过阀值long_query_time的SQL语句，通过日志去找出IO大的SQL以及发现未命中索引的SQL （默认查询时间超过10秒的会被记录到这个日志，需要手动到配置文件中开启）
